...
void			calc_intersection(float *delta, float3 *ray_pos, float3 *ray_dir, float3 *intersect)
{
...
short			plane_intersection(t_data *data, global t_obj *obj)
{
	...
	if (t < 0.0f)
		return (0);
	calc_intersection(&t, &data->ray_pos, &data->ray_dir, &data->intersect);
	return (1);
}
...
short			cone_intersection(t_data *data, global t_obj *obj)
{
	...
	calc_intersection(&delta, &data->ray_pos, &rdir, &data->intersect);
